...
{
    public class CentroidsController : ApiController
    {
        ...
        public HttpResponseMessage Get(String beam_modep)
        {
            ...
            String top = "{  \"type\": \"FeatureCollection\", \"name\": \"catalog_centroids\", \"crs\": { \"type\": \"name\", \"properties\": { \"name\": \"urn:ogc:def:crs:OGC:1.3:CRS84\" } },\"features\": [";
            sb.Append(top);
            ...
            String query = "SELECT row_to_json(f) As feature FROM(SELECT 'Feature' As type, ST_AsGeoJSON(\"wkb_geometry\")::json As geometry, row_to_json((SELECT l FROM(SELECT ogc_fid AS feat_id,titleprope,orbit_dirp,sensor_mop,polarizatp,orderablep,service_up,look_oriep,dateproper,end_datepr) As l)) As properties  FROM public.catalog_centroids As l " + whereClause.ToString() + " ) As f;";
